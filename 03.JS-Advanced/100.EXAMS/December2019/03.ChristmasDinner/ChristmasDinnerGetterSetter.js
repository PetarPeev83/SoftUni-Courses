class ChristmasDinner {
    constructor(budget) {
        this._budget = budget;
        this.budget = this._budget;
        this.dishes = [];
        this.dishesNames = [];
        this.products = [];
        this.guests = {};
        this.guestsNames = [];
    }
    get budget() {
        return Number(this._budget);
    };
    set budget(value) {
        if (value < 0) {
            throw new Error("The budget cannot be a negative number");
        } else {
            this._budget = value;
        };
    };
    shopping(product) {
        let type = product[0];
        let price = Number(product[1]);
        if (this.budget < price) {
            throw new Error("Not enough money to buy this product");
        } else {
            this.budget -= price;
            this.products.push(type);
            return `You have successfully bought ${type}!`;
        };
    };
    recipes(recipeObj) {
        let recipeName = recipeObj.recipeName;
        let productsList = recipeObj.productsList;
        for (let current of productsList) {
            if (!this.products.includes(current)) {
                throw new Error("We do not have this product");
            };
        };
        this.dishes.push(recipeObj);
        this.dishesNames.push(recipeName);
        return `${recipeName} has been successfully cooked!`;
    };
    inviteGuests(name, dish) {
        if (!this.dishesNames.includes(dish)) {
            throw new Error("We do not have this dish");
        };
        if (this.guestsNames.includes(name)) {
            throw new Error("This guest has already been invited");
        };
        this.guestsNames.push(name);
        // let toAdd = { [name]: dish };
        this.guests = Object.assign(this.guests , { [name]: dish });
        return `You have successfully invited ${name}!`;
    };
    showAttendance() {
        let result = [];
        for (let current in this.guests) {
            console.log(current);
            let name = current;
            let dish = this.guests[current];
            let dishIndex = this.dishesNames.indexOf(dish);
            let products = this.dishes[dishIndex].productsList.join(', ');
            result.push(`${name} will eat ${dish}, which consists of ${products}`);
        };
        return result.join('\n');
    };
};

let dinner = new ChristmasDinner(300);

dinner.shopping(['Salt', 1]);
dinner.shopping(['Beans', 3]);
dinner.shopping(['Cabbage', 4]);
dinner.shopping(['Rice', 2]);
dinner.shopping(['Savory', 1]);
dinner.shopping(['Peppers', 1]);
dinner.shopping(['Fruits', 40]);
dinner.shopping(['Honey', 10]);
// console.log(dinner.products)
dinner.recipes({
    recipeName: 'Oshav',
    productsList: ['Fruits', 'Honey']
});
dinner.recipes({
    recipeName: 'Folded cabbage leaves filled with rice',
    productsList: ['Cabbage', 'Rice', 'Salt', 'Savory']
});
dinner.recipes({
    recipeName: 'Peppers filled with beans',
    productsList: ['Beans', 'Peppers', 'Salt']
});
dinner.inviteGuests('Ivan', 'Oshav');
dinner.inviteGuests('Petar', 'Folded cabbage leaves filled with rice');
dinner.inviteGuests('Georgi', 'Peppers filled with beans');
// dinner.inviteGuests('Georgi', 'Peppers filled with beans');
// console.log(dinner.guests);
console.log(dinner.showAttendance());
